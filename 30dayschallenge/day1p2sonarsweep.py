...
#  The second window is marked B (200, 208, 210); its sum is 618. The sum of measurements in the 
...
#removing last empty entry in list 
y.pop()

previouswindow = []
currentwindow = []
largerbefore = 0
numbersy = [int(i) for i in y]

for i in range(len(y)):
    if i <= 2:
        continue
    previouswindow = [numbersy[i-3], numbersy[i-2], numbersy[i-1]]
    currentwindow = [numbersy[i-2], numbersy[i-1], numbersy[i]]
...
#     if i <= 2:
#         continue
#     previouswindow = [testlist[i-3], testlist[i-2], testlist[i-1]]
#     currentwindow = [testlist[i-2], testlist[i-1], testlist[i]]
#     if sum(currentwindow) > sum(previouswindow):
#         largerbefore += 1
